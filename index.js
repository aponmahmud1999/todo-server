...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.hxlcc.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.delete("/tasks/:id", async (req, res) => {
      ...
      app.put('/tasksstat/:id',async(req,res)=>{
        const id=req.params.id
        const updatedProduct=req.body
        const filter ={_id:ObjectId(id)}
        const options = { upsert: true }
